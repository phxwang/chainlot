...
//TODO: use async and await to refactor
var cltoken;
var chainlotticket;
var pool;
...
var afterNewPool=function(chainlot) {
	chainlot.chainlotPools(1).then(function(r) {
		pool = ChainLotPool.at(r);
		//console.log(ChainLotPool);
		console.log(pool);
		buyRandom(chainlot, 0, afterBuyRandom)(null);
	});
}
...
var afterAddBlockNumber=function(chainlot) {
	pool.prepareAwards({gas:5000000}).then(function(r){
...
var matchAwards=function(chainlot, index, thenFunc) {
	return function(r) {
		...
		if(index < 2) {
			pool.matchAwards(100, {gas:5000000}).then(matchAwards(chainlot, index+1, thenFunc));
		}
		else {
			thenFunc(chainlot);
		}
...
var afterMatchAwards=function(chainlot) {
	pool.calculateAwards(0, 100, {gas:5000000}).then(function(r){
		console.log("calculate awards, rule id 0");
		console.log(JSON.stringify(r.logs));
		pool.calculateAwards(1, 100, {gas:5000000}).then(function(r){
			console.log("calculate awards rule id 1");
			console.log(JSON.stringify(r.logs));
			pool.splitAward({gas:5000000}).then(function(r){
				...
				console.log(JSON.stringify(r.logs));
					pool.sendAwards({gas:5000000}).then(function(r){
						...
						cltoken.balanceOf(cltoken.address).then(function(r) {
							...
							pool.transferUnawarded(web3.eth.accounts[1], {gas:5000000}).then(function(r){
